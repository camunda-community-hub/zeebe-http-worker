 ...
 * 
 * Environment variables might need a M2M / JWT Token config to authorize to
 * load the variables
 *
 * <p>
...
public class RemoteEnvironmentVariablesProvider implements EnvironmentVariablesProvider {
	...
	public Map<String, String> getVariables() {
		...
		if (cachedVariables != null && Duration.between(lastUpdate, Instant.now()).toMillis() < config
				.getEnvironmentVariablesReloadInterval().toMillis()) {
		...
		try {
			...
			if (httpResponse.statusCode() != 200) {
				throw new RuntimeException(
						"Could not load environment variables successfully, see HTTP response with status "
			...
			throw new RuntimeException(
					"Could not load variables from '" + config.getEnvironmentVariablesUrl() + "': " + e.getMessage(),
					e);
		}
	}
	...
	private HttpRequest createHttpRequest() throws IOException, InterruptedException {
		...
		HttpRequest getVariablesRequest = HttpRequest.newBuilder().uri(URI.create(config.getEnvironmentVariablesUrl()))
				.headers(headers.toArray(new String[0])).GET().build();
	...
	private void refreshM2mToken() throws IOException, InterruptedException {
		...
		HttpRequest getTokenRequest = HttpRequest.newBuilder()
				...
				.headers("Accept", "application/json", "Content-Type", "application/json").build();
