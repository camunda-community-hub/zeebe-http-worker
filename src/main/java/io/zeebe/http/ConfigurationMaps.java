...
public class ConfigurationMaps {
	...
	public Optional<String> getStringIgnoreCase(String key) {
		return config.entrySet().stream().filter(entry -> entry.getKey().equalsIgnoreCase(key))
				.map(entry -> String.valueOf(entry.getValue())).filter(value -> !value.isEmpty()).findFirst();
	}
